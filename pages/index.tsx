  ...
  return <main style={styles.page}>
    ...
    <div style={styles.grid}>{devices.map((device) => {
      ...
      return <section key={device.id} style={{ ...styles.card, opacity: offline ? .72 : 1 }}><div style={styles.cardHeader}><div><h2 style={styles.cardTitle}>{device.name || device.id}</h2><span style={offline ? styles.offline : styles.online}>{offline ? '● Offline' : '● Online'}</span></div><strong style={styles.power}>{device.power ? 'ON' : 'OFF'}</strong></div>
        ...
        <div style={styles.temperature}><div style={styles.temperatureReading}><span style={styles.label}>Target temperature</span><strong style={styles.tempValue}>{device.targetTemperature === undefined ? '—' : `${device.targetTemperature}°`}</strong><small style={styles.temperatureHint}>Configured on the air conditioner</small></div><div style={styles.temperatureControl}><span style={styles.label}>Adjust target</span><div style={styles.stepper}><button style={styles.stepButton} aria-label="Decrease target temperature" disabled={offline || busy || temp <= 16} onClick={() => setTempEdits((items) => ({ ...items, [device.id]: clampTemp(temp - 1) }))}>−</button><output style={styles.stepValue} aria-label="Target temperature">{temp}<small>°C</small></output><button style={styles.stepButton} aria-label="Increase target temperature" disabled={offline || busy || temp >= 30} onClick={() => setTempEdits((items) => ({ ...items, [device.id]: clampTemp(temp + 1) }))}>+</button></div></div><button style={styles.apply} disabled={offline || busy || temp === device.targetTemperature} onClick={() => sendCommand(device.id, { temperature: temp })}>{busy ? 'Saving…' : 'Apply temperature'}</button></div>
      </section>;
    })}</div>
  </main>;
}
