...
export default function ApiKeysPage() {
  ...
  return <main style={styles.page}>
    ...
    <section style={styles.card}>
      <h2 style={styles.cardTitle}>API quick reference</h2>
      <p style={styles.muted}>Use an active key with a Shortcut or trusted automation. Keep it private and revoke it if exposed.</p>
      <div style={styles.docsGrid}>
        <div><span style={styles.docsLabel}>1. List devices</span><pre style={styles.snippet}>{`GET /api/devices\nAuthorization: Bearer YOUR_API_KEY`}</pre></div>
        <div><span style={styles.docsLabel}>2. Send a command</span><pre style={styles.snippet}>{`POST /api/shortcuts/command\nAuthorization: Bearer YOUR_API_KEY\nContent-Type: application/json\n\n{\n  "deviceId": "YOUR_DEVICE_ID",\n  "command": { "temperature": 23 }\n}`}</pre></div>
      </div>
      <p style={styles.muted}>Supported command fields: <code>power</code> (boolean), <code>temperature</code> (16–30), <code>mode</code> (for example <code>cool</code>), and <code>fanSpeed</code>.</p>
      <details style={styles.details}><summary style={styles.summary}>Example: turn the AC on</summary><pre style={styles.snippet}>{`{\n  "deviceId": "3460daa9564c",\n  "command": { "power": true }\n}`}</pre></details>
    </section>
  </main>;
}
